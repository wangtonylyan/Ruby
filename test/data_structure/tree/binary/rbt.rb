  ...
  module DataStructure
    class RedBlackTree
      ...
      def _check_root_(tree, left = 0, right = 0)
        ...
        raise "#{tree} | #{tree.left} | #{tree.right}" if
            tree.left && tree.left.color && tree.right && tree.right.color

        # the left and right subtree should hold the same black-height.
        ...
        # Due to the invariant that black-height is preserved in rotation operation,
        # the black-height of 'tree' is added by at most 1 only after it has been flipped.
        ...
        # the black-height of its parent, which takes both of them into count.
        # 2)
        # Whenever a new leaf is inserted as its parent's only child, its black-height should be 0,
